...
where
	...
	Executor: NativeExecutionDispatch + 'static,
{
	new_full_base::<RuntimeApi, Executor, _>(config, |_, _| ()).map(|NewFullBase { task_manager, .. }| {
		task_manager
	})
}
	...
	fn test_sync() {
		...
		sc_service_test::sync(
			...
			|config| {
				...
				} = new_full_base::<spanner_runtime::RuntimeApi, SpannerExecutor, _>(config,
					|
		...
		sc_service_test::consensus(
			...
			|config| {
				let NewFullBase { task_manager, client, network, transaction_pool, .. }
					= new_full_base::<spanner_runtime::RuntimeApi, SpannerExecutor, _>(config, |_, _| ())?;
