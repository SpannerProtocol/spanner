...
impl<'a> Iterator for BlockContentIterator<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((sender, signed_extra(0, spanner_runtime::ExistentialDeposit::get() + 1))),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive => {
						Call::Balances(
							BalancesCall::transfer_keep_alive(
								sp_runtime::MultiAddress::Id(receiver),
								spanner_runtime::ExistentialDeposit::get() + 1,
							)
						)
					},
					BlockType::RandomTransfersReaping => {
						Call::Balances(
							BalancesCall::transfer(
								...
								100*DOLLARS - (spanner_runtime::ExistentialDeposit::get() - 1),
							)
						)
					},
					BlockType::Noop => {
...
impl BenchKeyring {
	...
	pub fn generate_genesis(&self) -> spanner_runtime::GenesisConfig {
		crate::genesis::config_endowed(
			false,
			Some(spanner_runtime::wasm_binary_unwrap()),
